...
public class InventoryChecker {
    public static void checkInventory(MinecraftClient minecraftClient, boolean showMessage) {
        ...
        if (showMessage && VTiersClient.toggleAutoKitDetect) {
            VTiersClient.sendMessageToPlayer(Icons.colorText("Auto kit detect is enabled. Pressing the keybind won't make a difference", "green"), true);
            return;
        }

        Mode oldActivePvPTiersMode = VTiersClient.activePvPTiersMode;
        Mode detected = null;

        PlayerInventory playerInventory = minecraftClient.player.getInventory();

        if (checkVanilla(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_VANILLA;
            detected = Mode.VNLIST_VANILLA;
        }

        if (checkSword(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_SWORD;
            detected = Mode.VNLIST_SWORD;
        }

        if (checkUhc(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_UHC;
            detected = Mode.VNLIST_UHC;
        }

        if (checkPot(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_POT;
            detected = Mode.VNLIST_POT;
        }

        if (checkNethPot(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_NETHOP;
            detected = Mode.VNLIST_NETHOP;
        }

        if (checkSmp(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_SMP;
            detected = Mode.VNLIST_SMP;
        }

        if (checkAxe(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_AXE;
            detected = Mode.VNLIST_AXE;
        }

        if (checkMace(playerInventory)) {
            VTiersClient.activePvPTiersMode = Mode.VNLIST_MACE;
            detected = Mode.VNLIST_MACE;
        }

        if (oldActivePvPTiersMode != VTiersClient.activePvPTiersMode && detected != null) {
            ConfigManager.saveConfig();
            VTiersClient.sendMessageToPlayer(Text.empty().append(detected.getTextLabel()).append(Text.of(" was detected")), true);
        } else if (showMessage)
            VTiersClient.sendMessageToPlayer(Icons.colorText("No gamemode detected", "red"), true);
    }
