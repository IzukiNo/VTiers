package com.vtiers.screens;

import com.vtiers.PlayerProfileQueue;
import com.vtiers.VTiersClient;
import com.vtiers.misc.ConfigManager;
import com.vtiers.profile.PlayerProfile;
import com.vtiers.profile.Status;
import com.vtiers.profile.types.VNListProfile;
import com.vtiers.textures.ColorControl;
...
import static com.vtiers.VTiersClient.LOGGER;
...
//    private ButtonWidget centerPvPTiers;
    ...
    private ConfigScreen() {
        ...
        autoDetectKitBoundKey = String.valueOf(VTiersClient.autoDetectKey.getBoundKeyLocalizedText()).replace("literal{", "\"").replace("}", "\"");
        if (autoDetectKitBoundKey.length() != 3)
            autoDetectKitBoundKey = "the assigned keybind";

        cycleRightBoundKey = String.valueOf(VTiersClient.cycleRightKey.getBoundKeyLocalizedText()).replace("literal{", "\"").replace("}", "\"");
        if (cycleRightBoundKey.length() != 3)
            cycleRightBoundKey = "the assigned keybind";

        cycleLeftBoundKey = String.valueOf(VTiersClient.cycleLeftKey.getBoundKeyLocalizedText()).replace("literal{", "\"").replace("}", "\"");
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(textRenderer, VTiersClient.getRightIcon(), centerX + 90 + 32, distance + 75 + 9, Colors.WHITE);
        context.drawTextWithShadow(textRenderer, VTiersClient.getLeftIcon(), centerX - 90 - 32 - 12, distance + 75 + 9, Colors.WHITE);
...
//        centerPvPTiers.setPosition(centerX - 10, distance + 145);
    ...
    protected void init() {
        ...
        toggleMod = ButtonWidget.builder(Text.of(VTiersClient.toggleMod ? "Disable Tiers" : "Enable Tiers"), (buttonWidget) -> {
            VTiersClient.toggleMod();
            toggleTab.active = VTiersClient.toggleMod;
            toggleChat.active = VTiersClient.toggleMod;
            buttonWidget.setMessage(Text.of(VTiersClient.toggleMod ? "Disable Tiers" : "Enable Tiers"));
            buttonWidget.setTooltip(Tooltip.of(Text.of((VTiersClient.toggleMod ? "Disable Tiers" : "Enable Tiers"))));
        }).dimensions(width / 2 - 88 - 2, distance, 88, 20).tooltip(Tooltip.of(Text.of((VTiersClient.toggleMod ? "Disable Tiers" : "Enable Tiers")))).build();

        toggleIcons = ButtonWidget.builder(VTiersClient.toggleIcons ? Icons.ICONS : Icons.ICONS_DISABLED, (buttonWidget) -> {
            VTiersClient.toggleIcons();
            buttonWidget.setMessage(VTiersClient.toggleIcons ? Icons.ICONS : Icons.ICONS_DISABLED);
            buttonWidget.setTooltip(Tooltip.of(Text.of(VTiersClient.toggleIcons ? "Disable the gamemode icon next to the tier" : "Enable the gamemode icon next to the tier")));
        }).dimensions(width / 2 + 2, distance, 28, 20).tooltip(Tooltip.of(Text.of(VTiersClient.toggleIcons ? "Disable the gamemode icon next to the tier" : "Enable the gamemode icon next to the tier"))).build();

        toggleTab = ButtonWidget.builder(VTiersClient.toggleTab ? Icons.TAB : Icons.TAB_DISABLED, (buttonWidget) -> {
            VTiersClient.toggleTab();
            buttonWidget.setMessage(VTiersClient.toggleTab ? Icons.TAB : Icons.TAB_DISABLED);
            buttonWidget.setTooltip(Tooltip.of(Text.of(VTiersClient.toggleTab ? "Disable Tiers on the tablist" : "Enable Tiers on the tablist")));
        }).dimensions(width / 2 + 2 + 28 + 2, distance, 28, 20).tooltip(Tooltip.of(Text.of(VTiersClient.toggleTab ? "Disable Tiers on the tablist" : "Enable Tiers on the tablist"))).build();

        toggleChat = ButtonWidget.builder(VTiersClient.toggleChat ? Icons.CHAT : Icons.CHAT_DISABLED, (buttonWidget) -> {
            VTiersClient.toggleChat();
            buttonWidget.setMessage(VTiersClient.toggleChat ? Icons.CHAT : Icons.CHAT_DISABLED);
            buttonWidget.setTooltip(Tooltip.of(Text.of(VTiersClient.toggleChat ? "Disable Tiers in chat" : "Enable Tiers in chat")));
        }).dimensions(width / 2 + 2 + 28 + 2 + 28 + 2, distance, 28, 20).tooltip(Tooltip.of(Text.of(VTiersClient.toggleChat ? "Disable Tiers in chat" : "Enable Tiers in chat"))).build();

        toggleTab.active = VTiersClient.toggleMod;
        toggleChat.active = VTiersClient.toggleMod;

        toggleSeparatorMode = ButtonWidget.builder(Text.of(VTiersClient.toggleAdaptiveSeparator ? "Disable Dynamic Separator" : "Enable Dynamic Separator"), (buttonWidget) -> {
            VTiersClient.toggleAdaptiveSeparator();
            buttonWidget.setMessage(Text.of(VTiersClient.toggleAdaptiveSeparator ? "Disable Dynamic Separator" : "Enable Dynamic Separator"));
            buttonWidget.setTooltip(Tooltip.of(Text.of(VTiersClient.toggleAdaptiveSeparator ? "Make the Tiers separator gray" : "Make the Tiers separator match the tier color")));
        }).dimensions(width / 2 - 90, distance + 25, 180, 20).tooltip(Tooltip.of(Text.of(VTiersClient.toggleAdaptiveSeparator ? "Make the Tiers separator gray" : "Make the Tiers separator match the tier color"))).build();

        cycleDisplayMode = ButtonWidget.builder(Text.of(VTiersClient.displayMode.getCurrentMode()), (buttonWidget) -> {
            VTiersClient.cycleDisplayMode();
            buttonWidget.setMessage(Text.of(VTiersClient.displayMode.getCurrentMode()));
        ...
        autoKitDetect = ButtonWidget.builder(Text.of(VTiersClient.toggleAutoKitDetect ? "Disable auto kit detect" : "Enable auto kit detect"), (buttonWidget) -> {
            VTiersClient.toggleAutoKitDetect();
            buttonWidget.setMessage(Text.of(VTiersClient.toggleAutoKitDetect ? "Disable auto kit detect" : "Enable auto kit detect"));
            buttonWidget.setTooltip(Tooltip.of(Text.of((VTiersClient.toggleAutoKitDetect ?
        ...
        }).dimensions(width / 2 - 90, distance + 75, 180, 20).tooltip(Tooltip.of(Text.of((VTiersClient.toggleAutoKitDetect ?
                "Disable auto kit detect: you will need to press " + autoDetectKitBoundKey + " to auto-detect the current gamemode" :
        ...
        clearPlayerCache = ButtonWidget.builder(Text.of("Clear cache"), (buttonWidget) -> VTiersClient.clearCache(false)).dimensions(width - 88 - 5, height - 20 - 5, 88, 20).tooltip(Tooltip.of(Text.of("Clear all player cache"))).build();

        leftPvPTiers = ButtonWidget.builder(Text.of("←"), (buttonWidget) -> {
            VTiersClient.positionPvPTiers = VTiersClient.DisplayStatus.LEFT;
...
//            VTiersClient.positionPvPTiers = VTiersClient.DisplayStatus.OFF;
...
//        }).dimensions(centerX - 10, distance + 145, 20, 20).tooltip(Tooltip.of(Text.of("Disable PvPTiers"))).build();
        ...
        rightPvPTiers = ButtonWidget.builder(Text.of("→"), (buttonWidget) -> {
            VTiersClient.positionPvPTiers = VTiersClient.DisplayStatus.RIGHT;
//            updateRightSwitcher(buttonWidget, leftPvPTiers, centerPvPTiers);
        ...
        switch (VTiersClient.positionPvPTiers) {
...
//            case OFF -> centerPvPTiers.active = false;
        ...
        activeRightMode = ButtonWidget.builder(Icons.CYCLE, (buttonWidget) -> {
            VTiersClient.cycleRightMode();
            autoKitDetect.setMessage(Text.of(VTiersClient.toggleAutoKitDetect ? "Disable auto kit detect" : "Enable auto kit detect"));
            autoKitDetect.setTooltip(Tooltip.of(Text.of((VTiersClient.toggleAutoKitDetect ?
                    "Disable auto kit detect: you will need to press " + autoDetectKitBoundKey + " to auto-detect the current gamemode" :
                    "Enable auto kit detect: Tiers will always scan your inventory to display the right gamemode (instead of pressing " + autoDetectKitBoundKey + ")"))));
        ...
        ButtonWidget useClassicIcons = ButtonWidget.builder(VTiersClient.activeIcons == Icons.Type.CLASSIC ? Text.of("●") : Text.empty(), (buttonWidget) -> {
            buttonWidget.setMessage(VTiersClient.activeIcons == Icons.Type.CLASSIC ? Text.of("●") : Text.empty());
            VTiersClient.changeIcons(Icons.Type.CLASSIC, true);
        }).dimensions(5, 5, 20, 20).tooltip(Tooltip.of(Text.of("Use classic styled icons and colors"))).build();

        ButtonWidget usePvPTiersIcons = ButtonWidget.builder(VTiersClient.activeIcons == Icons.Type.PVPTIERS ? Text.of("●") : Text.empty(), (buttonWidget) -> {
            buttonWidget.setMessage(VTiersClient.activeIcons == Icons.Type.PVPTIERS ? Text.of("●") : Text.empty());
            VTiersClient.changeIcons(Icons.Type.PVPTIERS, true);
        }).dimensions(5, 30, 20, 20).tooltip(Tooltip.of(Text.of("Use PvPTiers styled icons and colors"))).build();

        ButtonWidget useMCTiersIcons = ButtonWidget.builder(VTiersClient.activeIcons == Icons.Type.MCTIERS ? Text.of("●") : Text.empty(), (buttonWidget) -> {
            buttonWidget.setMessage(VTiersClient.activeIcons == Icons.Type.MCTIERS ? Text.of("●") : Text.empty());
            VTiersClient.changeIcons(Icons.Type.MCTIERS, true);
        }).dimensions(5, 55, 20, 20).tooltip(Tooltip.of(Text.of("Use MCTiers styled icons and colors"))).build();

        switch (VTiersClient.activeIcons) {
...
//    }
    ...
    private void updateVisibilities() {
        activeRightMode.visible = VTiersClient.positionPvPTiers == VTiersClient.DisplayStatus.RIGHT;
        activeLeftMode.visible = VTiersClient.positionPvPTiers == VTiersClient.DisplayStatus.LEFT;
    }
    ...
    private void loadPlayerAvatar() {
        ...
        try (FileInputStream fileInputStream = new FileInputStream(FabricLoader.getInstance().getGameDir().resolve("cache/vtiers/" + (useOwnProfile ? ownProfile.uuid : defaultProfile.uuid) + ".png").toFile())) {
